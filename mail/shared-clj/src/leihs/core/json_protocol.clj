...
(clojure.core/extend-type org.joda.time.DateTime clojure.data.json/JSONWriter
                          (-write [date-time out options]
                            (clojure.data.json/write (date-time-to-string date-time) out options)))

(clojure.core/extend-type java.sql.Timestamp clojure.data.json/JSONWriter
                          (-write [sql-time out options]
                            (clojure.data.json/write (time-coerce/from-sql-time sql-time) out options)))

(clojure.core/extend-type java.time.Instant clojure.data.json/JSONWriter
                          (-write [inst out options]
                            (clojure.data.json/write (str inst) out options)))

(clojure.core/extend-type java.util.UUID clojure.data.json/JSONWriter
                          (-write [uuid out options]
                            (clojure.data.json/write (. uuid toString) out options)))
